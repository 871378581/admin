var DsyProvinces = new Array("北京市", "天津市", "上海市", "重庆市", "河北省", "山西省", "内蒙古", "辽宁省", "吉林省",
  "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西",
  "海南省", "四川省", "贵州省", "云南省", "西藏", "陕西省", "甘肃省", "青海省", "宁夏", "新疆", "香港", "澳门", "台湾省");
...
//省
Dsy.prototype.provinces = function () {
    return dsy.Items["0"];
};

//市
Dsy.prototype.city = function (province) {
    var index = dsy.Items["0"].indexOf(province);
    var item = dsy.Items["0" + "_" + index];
    if(!item) return [];
    return item;
};

//区
Dsy.prototype.district = function (province, city) {
    var p_index = dsy.Items["0"].indexOf(province);
    var c_index = dsy.Items["0" + "_" + p_index].indexOf(city);
    var item = dsy.Items["0" + "_" + p_index+"_" + c_index];
    if(!item) return [];
    return item;
};
